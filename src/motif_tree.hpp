...
inline size_t getCode(char base) {
   switch (base) {
     case 'A':
       return 0;
     case 'C':
...
union TreeNode {
  struct {
  TreeNode* children[4];  // TODO make dynamic
  size_t sharing_info;
  };
  TreeNode* next;
};
...
void print_node(TreeNode *t) {
  std::cout << "Node " << t << ", sharing info=" << t->sharing_info << std::endl;
...
TreeNode* free_head = 0;
TreeNode empty_node = { {{0}, 0} };

TreeNode* allocateNode() {
  if (!free_head) {
    free_head = new TreeNode[POOL_SIZE];
    TreeNode* head = free_head;
  ...
  TreeNode* head = free_head;
  free_head = head->next;
  memset((char*)head, 0, sizeof(TreeNode));
  return head;
}
...
void valid_recursive(const TreeNode *t) {
  valid(t);
...
void print_recursive(const TreeNode *t) {
  static int depth = 0;
...
class MotifTree {
  ...
  Motifs &motifs;
  size_t count;
  void traverseRecursive(TreeNode* node, size_t depth) {
  ...
  void copy(TreeNode* to, const TreeNode* from) {
    assert(from);
    size_t current_info = 0;
    size_t current_mask = 1;
    for (size_t j=0; j<domain_size; j++) {
      if (!(current_info & current_mask)) {
        if (from->children[j]) {
          TreeNode *new_node  = allocateNode();
  ...
  void copyUnion(TreeNode* to, const TreeNode* from, const std::string& motif, size_t depth) {
    //std::cout << "Copy Union: motif=" << motif << ", depth=" << depth << std::endl;
    if (depth == max_depth) return;
    size_t child = getCode(motif[depth]);
    if (child >= domain_size) {
      ...
      to->children[child] = allocateNode();
  ...
  bool emptyNode(const TreeNode* node) {
    for (size_t j=0; j<domain_size; j++) {
      if (node->children[j]) return false;
    }
    return true;
  }
  void copyIntersection(TreeNode* to, const TreeNode* from, const TreeNode* other, size_t depth) {
  ...
  void insertRecursive(TreeNode* node, const std::string& motif, size_t depth) {
  ...
  void insertCommonRecursive(TreeNode* to, const TreeNode* from, const std::string& motif, size_t depth) {
  ...
  std::string tmp(depth, '#');
    ...
    for (size_t j=min_j; j<=max_j; j++) {
      ...
      if ((current_info & current_mask) && from->children[j]) {
        ...
        if (common) {
          if (to_info != common) {
            TreeNode* old_node = to->children[j];
            TreeNode* new_node = allocateNode();
  ...
  void intersectRecursive(TreeNode* to, const TreeNode* from, size_t depth) {
    size_t current_info = mask;
    size_t current_mask = 1;
    for (size_t j=0; j<domain_size; j++) {
  ...
  bool hasIntersect(TreeNode* node, const std::string& motif, size_t depth) {
    if (node) {
      size_t child = getCode(motif[depth]);
      if (child >= domain_size) {
        size_t current_info = 0;
  ...
  public:
  MotifTree(size_t _max_depth, Motifs& _motifs, const char* _name) : root(allocateNode()), max_depth(_max_depth), x(max_depth, ' '), mask((1<<domain_size) - 1), name(_name), motifs(_motifs) {}
  ~MotifTree() {
  ...
  void insert(const MotifTree* other, const std::string& motif) {
    insertCommonRecursive(root, other->root, motif, 0);
  }
  void intersect(const MotifTree* other) {
    intersectRecursive(root, other->root, 0);
  }
  bool intersect(const std::string& x) {
    return hasIntersect(root, x, 0);
  }
  void print() {
...
//  mt2.insert("**G");
  ...
  mt.intersect(&mt2);
  mt.traverseOut();
//  std::cout << mt.intersect("TCA");
//  std::cout << mt.intersect("TCG");
}
